<template>
  <VerticalLayout>
    <b-row>
      <b-col>
        <UIComponentCard title="Tambah Ads Script">
          <b-row class="g-3">
            ...
            <b-col md="6">
              <b-form-group label="Client Secret" label-for="google-client-secret">
                <b-form-input id="google-client-secret" v-model="formState.google_client_secret" type="password" placeholder="Client secret..." />
              </b-form-group>
            </b-col>

            <b-col md="6">
              <b-form-group label="Refresh Token" label-for="google-refresh-token">
                <b-form-input id="google-refresh-token" v-model="formState.google_refresh_token" type="password" placeholder="Refresh token..." />
              </b-form-group>
            </b-col>
...
import { createAdsScript } from "@/services/adsScriptService";
...
const { mutate, isPending } = useMutation({
  mutationFn: () => {
    ...
    return createAdsScript(payload);
  },
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ["ads-scripts"] });
    showToast("Ads script berhasil ditambahkan", { type: "success", position: "top-center" });
    setTimeout(() => router.push("/ads-scripts"), 1500);
  },
  onError: (err: any) => {
    const msg = err?.response?.data?.message ?? "Gagal menyimpan ads script";
