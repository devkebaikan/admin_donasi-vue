<template>
  <VerticalLayout>
      ...
      <b-row>
        ...
        <b-col cols="12" md="3" class="mb-3">
          ...
          <b-form-select v-model="selectedStatus" @change="resetPage">
            ...
            <b-form-select-option value="PUBLISH">Publish</b-form-select-option>
    ...
    <b-row>
      <b-col>
        <UIComponentCard id="basic" title="Daftar Event Salur">
          <div v-if="isCanCreate" class="d-flex justify-content-end mb-3">
            <b-button
              variant="primary"
              @click="router.push('/event-salur/create')"
            >
              <i class="bx bx-plus fs-16 me-1"></i>Tambah Event Salur
            </b-button>
          </div>

          <div v-if="isLoading" class="text-center p-4">
            <b-spinner variant="primary" />
            <p class="mt-2">Memuat data event salur...</p>
          </div>

          <div v-else-if="isError" class="alert alert-danger">
            Error memuat data event salur: {{ error?.message || "Coba lagi." }}
          </div>
    ...
    <!-- Offcanvas Detail Event Salur & Kelola Project -->
    <EventSalurDetailOffcanvas
      v-model="showDetailOffcanvas"
      :event-salur-id="selectedEventSalurId"
    />
  </VerticalLayout>
</template>
...
import EventSalurDetailOffcanvas from "./components/EventSalurDetailOffcanvas.vue";
import { useEventSalurTable } from "./components/data";
import { hasPermission } from "@/helpers/permission";
import router from "@/router";

const isCanCreate = true;

const showDetailOffcanvas = ref(false);
const selectedEventSalurId = ref(0);
...
const handleGlobalClick = (event: Event) => {
  ...
  if (detailBtn) {
    ...
    if (id) {
      selectedEventSalurId.value = Number(id);
  ...
  if (editBtn) {
    ...
    if (id) router.push(`/event-salur/${id}/edit`);
  }
  if (deleteBtn) {
    event.preventDefault();
