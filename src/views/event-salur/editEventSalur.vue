<template>
  <VerticalLayout>
    <b-row>
      <b-col>
        <UIComponentCard
          id="event-salur-form-edit"
          :title="`Edit Event Salur${eventSalurData ? ': ' + eventSalurData.judul : ''}`"
        >
          <!-- Loading state -->
          <div v-if="isFetchingEventSalur" class="text-center py-5">
            <b-spinner variant="primary" class="me-2" />
            <span class="text-muted">Memuat data event salur...</span>
          </div>

          <!-- Error state -->
          <div v-else-if="fetchError" class="text-center py-5">
            <i class="bx bx-error-circle fs-1 text-danger d-block mb-2"></i>
            <p class="text-danger">Gagal memuat data event salur.</p>
            <b-button
              variant="outline-primary"
              size="sm"
              @click="refetchEventSalur"
            >
...
const eventSalurId = computed(() => Number(route.params.id));
...
const {
  data: eventSalurData,
  isLoading: isFetchingEventSalur,
  isError: fetchError,
  refetch: refetchEventSalur,
} = useQuery({
  queryKey: ["event-salur", eventSalurId],
  queryFn: () => getEventSalurById(eventSalurId.value),
  enabled: computed(() => !!eventSalurId.value),
});
...
watch(
  ...
  (data) => {
    ...
    formState.tanggal_mulai = data.tanggal_mulai ?? "";
    formState.tanggal_selesai = data.tanggal_selesai ?? "";
...
// ── Mutation ───────────────────────────────────────────────────────────────
const { mutate: updateEventSalurPayload, isPending } = useMutation({
  mutationFn: (payload: FormData) =>
    updateEventSalur(eventSalurId.value, payload),
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ["event-salur"] });
    queryClient.invalidateQueries({ queryKey: ["event-salur", eventSalurId] });
    showToast("Event salur berhasil diperbarui", {
      type: "success",
      position: "top-center",
    });
  ...
  onError: (err: any) => {
    const msg = err?.response?.data?.message ?? "Gagal memperbarui event salur";
...
const handleSubmit = async () => {
  ...
  updateEventSalurPayload(formData);
};
</script>
