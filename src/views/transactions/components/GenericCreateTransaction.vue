...
// Tipe transaksi dari tombol "Tambah Donasi/Event/Zakat" di list donatur
const TYPE_MAP: Record<string, { type: string; detailType: string }> = {
  donation: { type: "donation", detailType: "donation" },
  zakat: { type: "zakat", detailType: "zakat" },
  event: { type: "other", detailType: "event" },
};
...
// User yang di-prefill dari query param (?user_id=) — mungkin tidak ada di hasil search default
const prefilledUserOption = ref<{ value: number; text: string } | null>(null);

const userList = computed(() => {
  const base = userSearchOptions.value as {
    value: string | number;
    text: string;
  }[];
  if (
    prefilledUserOption.value &&
    !base.some((o: any) => Number(o.value) === prefilledUserOption.value!.value)
  ) {
    return [...base, prefilledUserOption.value];
  }
  return base;
});
...
onMounted(async () => {
  ...
  if (queryType && typeof queryType === "string" && TYPE_MAP[queryType]) {
    const mapped = TYPE_MAP[queryType];
    formState.type = mapped.type;
    transactionDetails.value[0].detail_type = mapped.detailType;
  }
