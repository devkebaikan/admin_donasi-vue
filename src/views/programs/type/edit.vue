<template>
  <VerticalLayout>
    <b-row>
      <b-col>
        <UIComponentCard
          :title="`Edit Tipe Program${typeData ? ': ' + typeData.nama : ''}`"
        >
          <div v-if="isLoadingType" class="text-center p-4">
            <b-spinner variant="primary" />
            <p class="mt-2">Memuat data...</p>
          </div>

          <div v-else-if="isErrorType" class="alert alert-danger">
          ...
          <form v-else @submit.prevent="handleSubmit">
            <b-row class="g-3">
              ...
              <!-- Info jumlah program (read-only) -->
              <b-col v-if="typeData?.programs_count !== undefined" cols="12">
                <div class="alert alert-info mb-0 py-2">
                  <i class="bx bx-info-circle me-1"></i>
                  Tipe ini digunakan oleh
                  <strong>{{ typeData.programs_count }}</strong> program.
                </div>
              </b-col>
...
const {
  ...
  queryKey: ["program-type", typeId],
  queryFn: () => getProgramTypeById(typeId),
  enabled: !!typeId,
});
...
watch(typeData, (data) => {
  ...
  formState.keterangan = data.keterangan ?? "";
});

const { mutate, isPending } = useMutation({
  mutationFn: () =>
    updateProgramType(typeId, {
      nama: formState.nama,
      slug: formState.slug,
  ...
  onSuccess: () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["program-type", typeId] });
    showToast("Tipe program berhasil diperbarui", {
      type: "success",
      position: "top-center",
    });
  ...
  onError: (err: any) => {
    const msg =
      err?.response?.data?.message ?? "Gagal memperbarui tipe program";
