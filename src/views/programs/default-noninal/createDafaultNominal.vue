<template>
  <VerticalLayout>
    <b-row>
      <b-col>
        <UIComponentCard id="nominal-form-create" title="Tambah Default Nominal">
          <b-row class="g-3">
            <!-- Nama Pilihan -->
            <b-col md="6">
            ...
            <b-col cols="12" v-if="isPending" class="text-center">
              ...
              <span class="text-muted">Menyimpan nominal...</span>
            </b-col>
...
import { createNominal } from "@/services/nominalService";
...
// ── Mutation ───────────────────────────────────────────────────────────────
const { mutate: createNominalPayload, isPending } = useMutation({
  mutationFn: () =>
    createNominal({
  ...
  onSuccess: () => {
    ...
    showToast("Nominal berhasil ditambahkan", {
      type: "success",
      position: "top-center",
    });
  ...
  onError: (err: any) => {
    const msg = err?.response?.data?.message ?? "Gagal menambahkan nominal";
