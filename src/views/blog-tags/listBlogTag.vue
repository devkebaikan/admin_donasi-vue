<template>
  <VerticalLayout>
    ...
    <b-row>
      <b-col>
        <UIComponentCard id="basic" title="Daftar Tag Blog">
          <div v-if="isCanCreate" class="d-flex justify-content-end mb-3">
            <b-button variant="primary" :to="{ name: 'blog-tag.create' }">
              <i class="bx bx-plus fs-16 me-1"></i>Buat Tag Blog
            </b-button>
          </div>
...
import { useBlogTagTable } from "./components/data";
...
const handleGlobalClick = (event: Event) => {
  ...
  if (editBtn) {
    ...
    router.push(`/blog-tag/${editBtn.getAttribute("data-id")}/edit`);
    return;
  }
