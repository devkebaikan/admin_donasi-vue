<template>
  <VerticalLayout>
    <b-row>
      <b-col>
        <UIComponentCard
          :title="`Edit Ajuan${ajuanData ? ': ' + ajuanData.account_behalf : ''}`"
        >
          <!-- Loading -->
          <div v-if="isLoadingAjuan" class="text-center py-5">
            <b-spinner variant="primary" class="me-2" />
            <span class="text-muted">Memuat data ajuan...</span>
          </div>

          <!-- Error -->
          <div v-else-if="isErrorAjuan" class="alert alert-danger">
              ...
              <b-col md="4">
                <b-form-group label="Tipe" label-for="type">
                  ...
                  >
                    ...
                    >
                    <b-form-select-option value="more"
                      >More</b-form-select-option
                    >
                  </b-form-select>
...
import { getAjuanById, updateAjuan } from "@/services/ajuanService";
...
const ajuanId = Number(route.params.id);
...
const {
  data: ajuanData,
  isLoading: isLoadingAjuan,
  isError: isErrorAjuan,
} = useQuery({
  queryKey: ["ajuan", ajuanId],
  queryFn: () => getAjuanById(ajuanId),
  enabled: !!ajuanId,
});

watch(ajuanData, (data) => {
  if (!data) return;
  formState.project_id = data.project_id ?? 0;
  formState.mitra_id = data.mitra_id ?? 0;
  formState.bank_reference_id = data.bank_reference_id ?? 0;
  formState.account_behalf = data.account_behalf ?? "";
  formState.account_number = data.account_number ?? "";
  formState.nominal_ajuan = data.nominal_ajuan ?? 0;
  formState.biaya = data.biaya ?? 0;
...
const { mutate, isPending } = useMutation({
  ...
  onSuccess: () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["ajuan", ajuanId] });
    showToast("Data ajuan berhasil diperbarui", {
      type: "success",
      position: "top-center",
    });
  ...
  onError: (err: any) => {
    const msg = err?.response?.data?.message ?? "Gagal memperbarui data ajuan";
