<template>
  <VerticalLayout>
    ...
    <b-row>
      <b-col>
        <UIComponentCard id="basic" title="Daftar Tema">
          <div v-if="isCanCreate" class="d-flex justify-content-end mb-3">
            <b-button variant="primary" @click="router.push('/tema/create')">
              <i class="bx bx-plus fs-16 me-1"></i>Tambah Tema
            </b-button>
          </div>

          <div v-if="isLoading" class="text-center p-4">
            <b-spinner variant="primary" />
            <p class="mt-2">Memuat data tema...</p>
          </div>

          <div v-else-if="isError" class="alert alert-danger">
            Error memuat data tema: {{ error?.message || "Coba lagi." }}
          </div>
    ...
    <!-- Offcanvas Detail Tema & Kelola Program -->
    <TemaDetailOffcanvas
      v-model="showDetailOffcanvas"
      :tema-id="selectedTemaId"
    />
  </VerticalLayout>
</template>
...
import TemaDetailOffcanvas from "./components/TemaDetailOffcanvas.vue";
import { useTemaTable } from "./components/data";
...
const handleGlobalClick = (event: Event) => {
  ...
  if (detailBtn) {
    ...
    if (id) {
      selectedTemaId.value = Number(id);
  ...
  if (editBtn) {
    ...
    if (id) router.push(`/tema/${id}/edit`);
  }
  if (deleteBtn) {
    event.preventDefault();
